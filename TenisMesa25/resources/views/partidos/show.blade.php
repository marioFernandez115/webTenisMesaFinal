...
@section('content')
    ...
    <div class="col-md-6">
        <p class="mb-2">
            <span class="fw-bold"><i class="bi bi-house-door-fill text-primary"></i> Local:</span>
            <span class="ms-1">{{ $partido->equipo_local }}</span>
        </p>
        <p class="mb-2">
            <span class="fw-bold"><i class="bi bi-person-badge-fill text-secondary"></i> Árbitro:</span>
            <span class="ms-1">{{ $partido->arbitro ?? 'No especificado' }}</span>
        </p>
    </div>
    <div class="col-md-6">
        <p class="mb-2">
            <span class="fw-bold"><i class="bi bi-people-fill text-danger"></i> Visitante:</span>
            <span class="ms-1">{{ $partido->equipo_visitante }}</span>
        </p>
        <p class="mb-2">
            <span class="fw-bold"><i class="bi bi-geo-alt-fill text-success"></i> Lugar del Partido:</span>
            <span class="ms-1">{{ $partido->estado === 'local' ? 'Casa' : ($partido->estado === 'visitante' ? 'Fuera' : 'No especificado') }}</span>
        </p>
    </div>
</div>
                        ...
                        <tbody>
                            ...
                            @foreach ($partido->detalles as $index => $detalle)
                                <tr>
                                    ...
                                    {{-- Jugador Local --}}
                                    <td>
    {{ $detalle->usuarioLocal ? $detalle->usuarioLocal->nombreyapellidos : 'Jugador no encontrado' }}
</td>
                                    {{-- Jugador Visitante --}}
                                    ...
                                    @can('edit-acta')
                                        <form action="{{ route('detalles.update', $detalle->id) }}" method="POST">
                                            ...
                                            @for ($i = 1; $i <= 6; $i++)
                                                <td>
                                                    <input type="text" name="juego_{{ $i }}" value="{{ $detalle->{'juego_' . $i} }}"
                ...
                {{-- Resultado Final --}}
                <p class="h5 fw-bold text-dark">Ganador: {{ $partido->ganador }}</p>
